...
| 模块 | 角色 | 连接方式 | 说明 |
|------|------|----------|------|
| [xiaozhi-esp32](https://github.com/78/xiaozhi-esp32) (ESP32-S3) | 在线 AI 对话 | WiFi (STA 模式) | 开源项目, 接入大模型实现自然语言对话、语音合成 |
| 天问 ASRPRO | 离线唤醒 + 本地指令 | USART3 (9600 baud) | 离线语音唤醒词检测, 基础控制指令识别 |

**工作流程:**
1. 天问 ASRPRO 持续监听环境，检测到唤醒词后发送 `0x07` 指令给 STM32
2. STM32 通知 xiaozhi-esp32 进入 AI 对话模式
3. xiaozhi-esp32 通过 WiFi 连接云端大模型，完成语音识别 (ASR) → 大模型推理 → 语音合成 (TTS) 全链路
4. 对话结束后，天问 ASRPRO 发送 `0x08` 指令，系统回到离线待唤醒状态

---

### 2.1 天问 ASRPRO 协议 (USART3, 9600 baud)

#### 2.1.1 下行指令 (ASRPRO → STM32)
